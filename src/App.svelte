<script>
  ...
  onMount(async () => {
    try {
      const res = await fetch("/test1.pdf");
  ...
  async function addPDF(file) {
    try {
      ...
      console.log("ดาวน์โหลด PDF ล้มเหลว.");
      throw e;
    }
  }
  ...
  async function addImage(file) {
    try {
      ...
      console.log(`ดาวน์โหลดภาพล้มเหลว.`, e);
    }
  }
  ...
  function addTextField(text = "เพิ่มข้อความ") {
    const id = genID();
    fetchFont(currentFont);
    const object = {
      id,
...
<main class="flex flex-col items-center py-16 bg-gray-100 min-h-screen">
  <div
    ...
    <input
      ...
      font-bold py-2 px-3 md:px-4 h-10 rounded mr-3 cursor-pointer md:mr-6 "
      for="pdf" title="เลือกไฟล์">
    ...
    <div
      class="relative mr-3 flex  h-12 bg-blue-200 rounded-sm overflow-hidden
      md:mr-4">
     
      <label
        class="flex items-center justify-center h-12 w-12 bg-blue-200 hover:bg-blue-300
        cursor-pointer py-1 px-1 rounded mr-4" title="เพิ่มรูปภาพ"
        for="image"
        class:cursor-not-allowed={selectedPageIndex < 0}
        class:bg-blue-500={selectedPageIndex < 0}>
        <img src="i1.png" alt="An icon for adding images" />
      </label>
      <label
        class="flex items-center justify-center h-12 w-12 bg-blue-200 hover:bg-blue-300
        cursor-pointer py-3 px-1 rounded mr-4" title="เพิ่มข้อความ"
        for="text"
        class:cursor-not-allowed={selectedPageIndex < 0}
        class:bg-blue-500={selectedPageIndex < 0}
        on:click={onAddTextField}>
        <img src="t1.png" alt="An icon for adding text" />
      </label>
      <label
        class="flex items-center justify-center h-12 w-12  bg-blue-200 hover:bg-blue-300
        cursor-pointer py-3 px-1 rounded mr-4" title="เพิ่มลายเซ็น"
        on:click={onAddDrawing}
        class:cursor-not-allowed={selectedPageIndex < 0}
        class:bg-blue-500={selectedPageIndex < 0}>
        <img src="s2.png" alt="An icon for adding drawing" />
      </label>

    </div>
    <div class="justify-center mr-2 md:mr-3 w-full max-w-xs hidden md:flex ">
      <input
        placeholder =" เปลี่ยนชื่อไฟล์ PDF ที่นี่"
        type="text"
    ...
    <label
      on:click={savePDF}
      class="flex items-center justify-center h-12 w-12 hover:bg-blue-300
        cursor-pointer py-1 px-1 rounded mr-4" title="บันทึก"
      class:cursor-not-allowed={pages.length === 0 || saving || !pdfFile}
      class:bg-blue-700={pages.length === 0 || saving || !pdfFile}>
      <!-- {saving ? 'Saving' : 'บันทึก'} -->
      <img src="save-file.png" alt="An icon for adding drawing" />
  </label>
  ...
  {#if addingDrawing}
    <div
      ...
      class="fixed z-10 top-0 left-0 right-0 border-b border-blue-300 bg-white
      shadow-lg"
      style="height: 50%;">
      <DrawingCanvas
        on:finish={e => {
  ...
  {#if pages.length}
    <div class="flex justify-center px-5 w-full md:hidden">
      <img src="/edit.svg" class="mr-2 py-6" alt="a pen, edit pdf name" />
      <input
        placeholder="เปลี่ยนชื่อไฟล์ PDF กดที่นี่"
        type="text"
        class="flex-grow bg-transparent"
        bind:value={pdfName} />
    </div>
  ...
  {:else}
    <div class="w-full flex-grow flex justify-center items-center">
      <span class=" font-bold text-3xl text-gray-500">ลากไฟล์ PDF วางที่นี่</span>
    </div>
  {/if}
</main>
